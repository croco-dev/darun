...
export const ProductPhotos = bind(useProductPhotos, ({ photos }: ProductPhotosViewProps) => {
  ...
  if (!photos || photos.length === 0) {
    return (
      <div className="rounded-card border border-dark-100 px-4 py-4 shadow-button">
        <p className="text-sm text-dark-500">{t('photo.empty')}</p>
      </div>
    );
  }
  return (
    <div className="rounded-card border border-dark-100 px-2 py-2 shadow-button">
      {photos && (
          ...
          {photos.map(photo => (
            <Zoom key={photo.imageUrl}>
              <Image
                ...
                sizes="350px"
                fill={true}
                className="!relative !h-[220px] !w-auto rounded border border-dark-100 object-contain"
              />
            </Zoom>
          ))}
        </div>
