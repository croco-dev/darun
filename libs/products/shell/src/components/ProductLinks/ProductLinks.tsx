        ...
        <Button variant="shadow" color={index === 0 ? 'primary' : 'secondary'}>
          <div className="flex items-center justify-center gap-2 px-0.5 py-0.5">
            ...
            <div className="flex flex-col items-start gap-0.5">
              <span className={`w-max break-keep text-[15px] font-medium ${index === 0 ? '' : 'text-dark-900'}`}>
                {link.title}
              </span>
              <span className={`break-keep text-[13px] font-medium ${index === 0 ? 'text-dark-200' : 'text-dark-400'}`}>
                {link.displayLink}
              </span>
            </div>
          </div>
