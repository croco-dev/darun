          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
            {categories.map(({ key, label, productCount, icon: Icon }) => (
              <Link
                key={key}
                href={`/categories/${key}`}
                className="group flex min-h-[144px] flex-col justify-between rounded-card border border-surface-300 bg-white p-4 shadow-sm transition-all duration-200 ease-out hover:-translate-y-1 hover:border-brand-300 hover:shadow-card-hover focus-visible:-translate-y-1 focus-visible:border-brand-300 focus-visible:shadow-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-300/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--home-bg-section-alt)] active:translate-y-0 active:scale-[0.99] motion-reduce:transform-none motion-reduce:transition-none sm:p-5"
              >
