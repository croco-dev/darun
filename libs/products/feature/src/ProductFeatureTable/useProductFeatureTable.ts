...
export function useProductFeatureTable({ slug, editFeature }: ProductFeatureTableProps) {
  const { data, loading, error, refetch } = useQuery(TempProductBySlugOnProductFeatureTableDocument, {
    variables: { slug },
  });
  return { features: data?.tempProductBySlug?.features, loading, error, editFeature, refetch };
}
